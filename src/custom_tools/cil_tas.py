...
class CIL_TAS_Tool(SingleMessageCustomTool):
    def __init__(self, data: pd.DataFrame = None):
        self.global_annual_data = data if data is not None else pd.read_csv("../Datasets/cil_global_tas_annual.csv")
        self.global_jja_data = data if data is not None else pd.read_csv("../Datasets/cil_global_tas_JJA.csv")
        self.global_djf_data = data if data is not None else pd.read_csv("../Datasets/cil_global_tas_DJF.csv")
        self.global_u32_data = data if data is not None else pd.read_csv("../Datasets/cil_global_tasmin_under32F.csv")
        self.global_o95_data = data if data is not None else pd.read_csv("../Datasets/cil_global_tasmax_over95F.csv")
        self.usa_annual_data = data if data is not None else pd.read_csv("../Datasets/cil_usa_tas_annual.csv")
        self.usa_jja_data = data if data is not None else pd.read_csv("../Datasets/cil_usa_tas_JJA.csv")
        self.usa_djf_data = data if data is not None else pd.read_csv("../Datasets/cil_usa_tas_DJF.csv")
        self.usa_u32_data = data if data is not None else pd.read_csv("../Datasets/cil_usa_tasmin_under32F.csv")
        self.usa_o95_data = data if data is not None else pd.read_csv("../Datasets/cil_usa_tasmax_over95F.csv")
    ...
    def get_description(self) -> str:
        ...
        such as season, country, sates of USA, years, SSP ranges, and percentiles. Additionally, it allows 
        ...
        return (
            ...
            "Users can filter the data by season, country, states of USA, year range, SSP range, and percentiles. "
    ...
    def get_parameters(self) -> Dict[str, ToolParamDefinitionParam]:
        ...
        return {
            "level": ToolParamDefinitionParam(
                type="str",
                description=(
                    "Dataset level indicating the global level or USA country level data subset to use. Options are:\n"
                    "'global' - Global data\n"
                    "'usa' - United States of America state level data"
                ),
                required=True
            ),
            "flag": ToolParamDefinitionParam(
                ...
                description=(
                    ...
                    "None - Use annual data"
                ),
                required=False
            ),
            "names": ToolParamDefinitionParam(
                type="list[str]",
            ...
            "group_by": ToolParamDefinitionParam(
                type="List[str]",
                description=(
                    "Column name(s) to group the data by (e.g., ['name', 'ssp_range'])."
                    "Avialable group_by options are: 'name', 'ssp_range', 'year_range'."
                ),
                required=False
            ),
    ...
    async def run_impl(
        ...
        try:
            if level == 'global':
                if flag == 'jja':
                    filtered_data = self.global_jja_data.copy()
                elif flag == 'djf':
                    filtered_data = self.global_djf_data.copy()
                elif flag == 'u32':
                    filtered_data = self.global_u32_data.copy()
                elif flag == 'o95':
                    filtered_data = self.global_o95_data.copy()
                else:
                    filtered_data = self.global_annual_data.copy()
            elif level == 'usa':
                if flag == 'jja':
                    filtered_data = self.usa_jja_data.copy()
                elif flag == 'djf':
                    filtered_data = self.usa_djf_data.copy()
                elif flag == 'u32':
                    filtered_data = self.usa_u32_data.copy()
                elif flag == 'o95':
                    filtered_data = self.usa_o95_data.copy()
                else:
                    filtered_data = self.usa_annual_data.copy()
            else:
        ...
        if(level in ['global', 'usa']):

            print("Level: ", level, "\nData:\n", filtered_data)

            # Step 1: Filter by names
            if names:
                available_names = filtered_data['name'].unique()
                missing_names = [c for c in names if c not in available_names]
                if missing_names:
                    messages.append(f"Data for the following isn't available: {', '.join(missing_names)}")
                filtered_data = filtered_data[filtered_data['name'].isin(names)]
            
            print("Step 1 Filtered Data by names \n", filtered_data.head(5))
            ...
            selected_columns = ['name', 'ssp_range', 'year_range']
